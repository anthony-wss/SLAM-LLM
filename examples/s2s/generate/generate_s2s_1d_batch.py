...
def main(kwargs: DictConfig):
	...
	decoding_strategy_1d = kwargs.decoding_strategy_1d
	...
	with open(pred_path, "w") as pred, open(gt_path, "w") as gt, open(question_path, "w") as q:
		for step, batch in enumerate(test_dataloader):
			...
			if decoding_strategy_1d == "text-then-speech":
				# (Anthony) We assume batch size is 1 here
				model_outputs = model_outputs.view(-1).tolist()
				try:
					audio_start_pos = model_outputs.index(model.model_config.vocab_config.eot) + 1
				except ValueError as e:
					logger.warning(f"Model does not output EOT token, so we cannot find audio tokens\nError: {e}")
				text_outputs = model_outputs[:audio_start_pos]
				audio_outputs = model_outputs[audio_start_pos:]
			else:
				logger.error(f"Decoding strategy {decoding_strategy_1d} invalid")
				raise NotImplementedError

			# (Anthony) Manually add EOA token if does not exist
			if audio_outputs[-1] != model.model_config.vocab_config.eoa:
				audio_outputs.append(model.model_config.vocab_config.eoa)
			...
			# (Anthony) We will always generate the audio for 1D models
			...
			for i, key in enumerate(batch["keys"]):
				audio_tokens = [torch.tensor(audio_outputs)]
