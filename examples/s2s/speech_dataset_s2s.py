...
class SpeechDatasetJsonl(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        assert self.manifest_format in ["parquet", "jsonl", "parquet_with_context", "parquet_with_context_mix"], "manifest_format must be one of [parquet, jsonl, parquet_with_context, parquet_with_context_mix]"
        ...
        if self.manifest_format == "parquet" or self.manifest_format == "parquet_with_context":
        ...
        elif self.manifest_format == "parquet_with_context_mix":
            from datasets import load_dataset, load_from_disk, concatenate_datasets
            paths = dataset_config.train_data_path.split(",")
            ds = []
            for path in paths:
                if dataset_config.load_from_cache_file:       
                    ds.append(load_dataset(path))       # load_from huggingface datasets
                else:
                    ds.append(load_from_disk(path))   # load_from local disk
            
            ds = concatenate_datasets(ds).shuffle(seed=self.seed)
            print("anthony debug we mix the data", flush=True)
            ...
            raise ValueError("manifest_format must be one of [parquet, jsonl, parquet_with_context, parquet_with_context_mix]")
    ...
    def extract_audio_feature(self, audio_path):
        ...
        if (self.manifest_format == "parquet" or self.manifest_format == "parquet_with_context" or self.manifest_format == "parquet_with_context_mix") and isinstance(audio_path, dict) or isinstance(audio_path, AudioDecoder):
    ...
    def __getitem__(self, index):
        ...
        elif self.manifest_format == "parquet_with_context" or self.manifest_format == "parquet_with_context_mix":
