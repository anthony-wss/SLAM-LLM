...
class SpeechDatasetJsonl(torch.utils.data.Dataset):
    ...
    def extract_audio_feature(self, audio_path):
        ...
        if self.manifest_format == "parquet" and isinstance(audio_path, dict):
            ...
            if self.code_type == "SNAC":
                ...
                audio_length = len(audio_tokens) + 1  # 1 is the eoa token
                audio_res = audio_tokens + [self._eoa]
    ...
    def get_input_ids(self, length, special_token_a, special_token_t):
        # Construct [audio tokens] * length tensor
        input_ids = []
        input_ids.append(self._input_a)
        input_ids += [self._pad_a] * length
        input_ids += [self._eoa, special_token_a]
        return input_ids

    def get_answer_ids(self, length):
        # (Anthony) For text-then-speech, the length is text respone + audio response
        # We use _pat_t as a hack to represent text and audio here.
        answer_ids = []
        answer_ids += [self._pad_t] * length
        return answer_ids
    ...
    def __getitem__(self, index):
        ...
        # (Anthony) We keep `prompt_ids` as 1-D list
        ...
        example_mask = torch.zeros_like(example_ids).ge(-1)  # [True,True]
        ...
        answer_text_length = len(answer_text_ids)
        ...
        # (Anthony) interleaving
        target_audio = target_audio[:-1]  # Remove eoa
        target_audio_length -= 1
        answer_ids = []
        text_inter_num, audio_inter_num = self.dataset_config.text_inter_num, self.dataset_config.audio_inter_num
        text_pos, audio_pos = 0, 0
        while True:
            if text_pos < answer_text_length:
                answer_ids += answer_text_ids[text_pos:text_pos+text_inter_num]
                text_pos += text_inter_num
            if audio_pos < target_audio_length:
                answer_ids += target_audio[audio_pos:audio_pos+audio_inter_num]
                audio_pos += audio_inter_num
            if text_pos >= answer_text_length and audio_pos >= target_audio_length:
                break

        answer_ids = torch.tensor(answer_ids, dtype=torch.int64)
        example_ids = torch.cat((example_ids, answer_ids), dim=0)

        labels_ids = copy.deepcopy(example_ids)
        labels_ids[:input_length + prompt_length + 3] = -1
    ...
    def padding(cls, sequence, padding_length, padding_idx=0, padding_side="right"):
        if isinstance(sequence, (int, list, tuple)):
            ...
            if sequence.ndimension() == 2:
                ...
                if padding_length >= 0:
                    if padding_side == "left":
                        sequence = torch.cat((torch.full([padding_length], padding_idx), sequence))
                    else:
                        sequence = torch.cat((sequence, torch.full([padding_length], padding_idx)))
                else:
                    sequence = sequence[:padding_length]
        elif isinstance(sequence, np.ndarray):
            if padding_length >= 0:
