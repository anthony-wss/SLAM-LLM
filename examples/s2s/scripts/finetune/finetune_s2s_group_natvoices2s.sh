...
train_data_path=/work/u3937558/natvoices2s/natvoices2s_slamllm.jsonl
val_data_path=/work/u3937558/natvoices2s/natvoices2s_slamllm.jsonl
...
# log settings
exp_name="s2s_natvoices2s-${llm_name}-gpu${num_gpus}-btz${batch_size_training}-lr${lr}-fp16-epochs${num_epochs}-whisper_${whisper_size}-group${code_layer}"
if [ "$use_fp16" != true ]; then
    exp_name="s2s_natvoices2s-${llm_name}-gpu${num_gpus}-btz${batch_size_training}-lr${lr}-nofp16-epochs${num_epochs}-whisper_${whisper_size}-group${code_layer}"
fi
